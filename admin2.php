...
<div class="jumbotron" style="background:#FFF" >
	...
	<form method="get">
		<p>ค้นหางานวิจัย : 
			<select class="chosen" style="width:300px;" name="research" id="research">
				<option value="0" <?php echo "selected"; ?>>โปรดเลือกหรือพิมพ์</option>
				<?php while($row_research = mysqli_fetch_array($query_research)):;?>
				<option value="<?php echo "$row_research[res_id]";?>" 
				<?php if($research_id == $row_research[res_id]){echo "selected";} ?>>
				<?php echo "$row_research[res_name]";?></option>
				<?php endwhile;?>
			</select>
    ...
    <table height="47" class="table table-bordered table-striped" id="mytable">
		<thead style="text-align: center; background: #24afd8"><br><br>
			<tr>
				...
				<td><?php if(!$row["res_name"]){echo " - ";} echo "{$row["res_name"]}"; ?></td>
			</tr>
			<tr>
				<th width="20%">ระดับคุณภาพงานวิจัย</th>
				<td><?php if(!$row["researcher"]){echo " - ";} echo "{$row["researcher"]}"; ?></td>
			</tr>
			<tr>
	...
	if ($result->num_rows > 0 && $result->num_rows <= 1000) {
    	...
    	<tbody>
			<tr align="center">
				...
				<td><?php if(!$row["res_timeline"]){echo " - ";} echo "{$row["res_timeline"]}"; ?></td>
				<td><?php if(!$row["res_member"]){echo " - ";} echo "{$row["res_member"]}"; ?></td>
				<td><?php if(!$row["res_detail"]){echo " - ";} echo "{$row["res_detail"]}"; ?></td>
				<td align="center">
